...
router.get("/user/tips", authMiddleware, async (req, res, next) => {
  try {
    ...
    return res.status(400).send({ message: "Something went wrong in /tips/user!" });
  }
});
router.post("/tips", authMiddleware, async (req, res, next) => {
  try {
    ...
    if (!text) return res.status(400).status({ message: "Please provide a placeId and text" });
    ...
    if (result) return res.status(200).send({ message: "Tip created!", data: result });
  } catch (e) {
    return res.status(400).send({ message: "Something went wrong in POST /tips!" });
  }
});
